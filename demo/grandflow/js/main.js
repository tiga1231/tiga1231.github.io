...
let canvas = d3.select('#bg')
.attr('width', window.innerWidth * window.devicePixelRatio)
.attr('height', window.innerHeight * window.devicePixelRatio)
.style('width', '100%')
.style('height', '100%');

let [gl, program] = initGL('#bg', ['demo/grandflow/shader/vertex.glsl', 'demo/grandflow/shader/fragment.glsl']);
...
function setDirection(theta){
  gl.uniform1f(gl.locs.du, Math.cos(theta));
  gl.uniform1f(gl.locs.dv, Math.sin(theta));
}


let theta = Math.random() * Math.PI;
let t0 = performance.now();
function directionTick(t){
  let dt = t-t0;
  t0 = t;
  theta += dt/1000 / 100;
  setDirection(theta);
  requestAnimationFrame(directionTick);
}
directionTick(t0);
